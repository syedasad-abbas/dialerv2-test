...
describe('UA', () => {
    ...
    test('UA no WS connection', () => {
        ...
        for (const parameter in consts.UA_CONFIGURATION_AFTER_START) {
            if (Object.prototype.hasOwnProperty.call(consts.UA_CONFIGURATION_AFTER_START, parameter)) {
                switch (parameter) {
                    case 'uri':
                    case 'registrar_server': {
                        // eslint-disable-next-line jest/no-conditional-expect
                        expect(ua.configuration[parameter].toString()).toBe(consts.UA_CONFIGURATION_AFTER_START[parameter]);
                        break;
                    }
                    case 'sockets': {
                    ...
                    default: {
                        // eslint-disable-next-line jest/no-conditional-expect
                        expect(ua.configuration[parameter]).toBe(consts.UA_CONFIGURATION_AFTER_START[parameter]);
                    }
                }
            }
        }
        ...
        expect(() => ua.sendMessage('sip:ibc@iñaki.ðđß', 'FAIL WITH INVALID_TARGET PLEASE')).toThrow();
        ua.stop();
    });
});
